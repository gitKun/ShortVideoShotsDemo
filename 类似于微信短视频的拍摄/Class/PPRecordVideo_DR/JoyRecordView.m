...
@property (nonatomic, copy) void(^completedAction)(BOOL isImage, NSURL *videoUrl);
...
-(UIButton *)startRecordBtn{
    if (!_startRecordBtn) {
        ...
        [_startRecordBtn addTarget:self action:@selector(takePhoto:) forControlEvents:UIControlEventTouchUpInside];
        
        //button长按事件
        
        UILongPressGestureRecognizer *longPress = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(startRecordBtnLongPress:)];
        longPress.minimumPressDuration = 0.5; //定义按的时间
        [_startRecordBtn addGestureRecognizer:longPress];
...
//                   );
    ...
    [self.cancleBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.mas_equalTo(weakSelf.startRecordBtn.mas_left).offset(-40);
    ...
    [self.progressView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.center.mas_equalTo(weakSelf.startRecordBtn);
...
+ (void)clearAllCacheData {
    NSString *cacheFolder = [JoyMediaRecordPlay cacheFileFolder];
...
- (void)startRecordBtnLongPress:(UILongPressGestureRecognizer *)gestureRecognizer {
...
- (void)takePhoto:(UIButton *)btn {
    // 拍照
    [self.recorder takePhotoComplete:^(UIImage *image) {
        [self receivePhotoWithImage:image];
    }];
}
...
- (void)leaveout:(UIButton *)btn{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [UIView animateWithDuration:0.3 animations:^{
            weakSlef.jk_top = CGRectGetHeight([UIScreen mainScreen].bounds);
...
- (void)mergeUrl:(NSURL *)recordUrl{
    ...
    [JoyMediaRecordPlay mergeAndExportVideosAtFileURLs:recordUrl newUrl:mergeUrlStr widthHeightScale:CGRectGetWidth([UIScreen mainScreen].bounds)/CGRectGetHeight([UIScreen mainScreen].bounds) presetName:AVAssetExportPresetHighestQuality mergeSucess:^{
        [weakSelf.recorder removeAVCaptureAudioDeviceInput];
        [weakSelf reviewRecordVideoWithUrl:[NSURL fileURLWithPath:mergeUrlStr]];
    }];
}

- (void)receivePhotoWithImage:(UIImage *)image {
    JoyPlayerView * playView= [[JoyPlayerView alloc]initWithFrame:self.bounds];
    playView.backgroundColor = [UIColor blackColor];
    playView.image = image;
    playView.playUrl = nil;
    ...
    playView.saveResourceComplete = ^(BOOL iSimage, NSURL *sourceUrl) {
        [weakSelf leaveout:nil];
        if (weakSelf.completedAction) {
            weakSelf.completedAction(iSimage, sourceUrl);
        }
    };
}
...
- (UIButton *)cancelButton
{
    if (!_cancelButton) {
        ...
        _cancelButton.jk_bottom = CGRectGetHeight([UIScreen mainScreen].bounds) - 60;
        _cancelButton.jk_centerX = self.jk_centerX;
...
- (UIButton *)doneButton
{
    if (!_doneButton) {
        ...
        _doneButton.jk_bottom = CGRectGetHeight([UIScreen mainScreen].bounds) - 60;
        _doneButton.jk_centerX = self.jk_centerX;
...
- (void)saveToPhotoBtnClick {
    __weak typeof(self) weakSelf = self;
    if (_image) {
        UIImageWriteToSavedPhotosAlbum(_image, weakSelf, @selector(image:didFinishSavingWithError:contextInfo:), nil);
        NSData *imageData = UIImageJPEGRepresentation(_image, 1.0);
        NSString *imagePath = [JoyMediaRecordPlay photoFilePathWithType:@"JPEG"];
        [imageData writeToFile:imagePath atomically:YES];
        _imageUrl = [NSURL fileURLWithPath:imagePath];
    } else if(_playUrl) {
        [JoyMediaRecordPlay saveToPhotoWithUrl:_playUrl complete:^(BOOL success, NSError * _Nullable error) {
            if(success) {
                [weakSelf saveResourceSuccess];
            }
        }];
    }
}
...
- (void)saveResourceSuccess {
    if(self.saveResourceComplete) {
        BOOL isImage = _imageUrl != nil && _playUrl == nil;
        self.saveResourceComplete(isImage, isImage ? _imageUrl : _playUrl);
    }
}
...
- (void)setImage:(UIImage *)image {
    ...
    __weak __typeof(&*self)weakSelf = self;
    [UIView animateWithDuration:0.5 animations:^{
        weakSelf.cancelButton.jk_centerX = weakSelf.jk_centerX-100;
        weakSelf.doneButton.jk_centerX = weakSelf.jk_centerX+100;
    }];
}
