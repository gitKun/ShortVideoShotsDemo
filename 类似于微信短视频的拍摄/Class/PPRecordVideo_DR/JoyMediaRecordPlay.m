...
#pragma mark - 拍照
- (void)takePhotoComplete:(void (^)(UIImage *))complete {
 ...
 */
- (void)changeDeviceProperty:(void(^)(id idObject))propertyChange
{
...
- (void)getVideoAuth:(void(^)(BOOL boolValue))videoAuth {
...
- (void)authAudio:(void(^)(BOOL boolValue))audio {
...
- (void)showAlert{
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"请在iPhone的“设置-隐私”选项中，允许%@访问你的摄像头和麦克风。" message:nil preferredStyle:UIAlertControllerStyleAlert];
    ...
    UIViewController *top = [window jk_currentViewController];
    [top presentViewController:alertController animated:YES completion:nil];
}


#pragma mark 视频保存相册
+ (void)saveToPhotoWithUrl:(NSURL *)url complete:(void (^)(BOOL, NSError * _Nullable))complete {
 ...
 */
+ (void)mergeAndExportVideosAtFileURLs:(NSURL *)fileURL newUrl:(NSString *)mergeFilePath widthHeightScale:(CGFloat)whScalle presetName:(NSString *)presetName mergeSucess:(void(^)(void))mergeSucess
...
+(void)mergingVideoWithmergeFilePath:(NSString *)mergeFilePath
               layerInstructionArray:(NSMutableArray*)layerInstructionArray
                    ...
                    widthHeightScale:(CGFloat)whScalle
                         ...
                         mergeSucess:(void(^)(void))mergeSucess
...
+ (NSString *)photoFilePathWithType:(NSString *)fileType {
    return  [[[self class] getVideoPathCache] stringByAppendingString:[[self class] getPhotoNameWithType:fileType]];
}
...
{
    NSString * timeStr = [self timingMarkString];
    NSString *fileName = [NSString stringWithFormat:@"/video_%@.%@",timeStr,fileType];
    return fileName;
}

+ (NSString *)getPhotoNameWithType:(NSString *)fileType {
    NSString * timeStr = [self timingMarkString];
    NSString *fileName = [NSString stringWithFormat:@"/IMG_%@.%@",timeStr,fileType];
    return fileName;
}

+ (NSString *)timingMarkString {
